...
test('publishes immutable versions and rolls active pointer back', () => {
  ...
  store.publish({id:'v1', topology:{id:'t',version:'1',entry:[],nodes:[],edges:[]}});
  store.publish({id:'v2', topology:{id:'t',version:'2',entry:[],nodes:[],edges:[]}});
  store.activate('v2');
  assert.equal(store.active().id, 'v2');
  store.rollback('v1');
  assert.equal(store.active().id, 'v1');
  assert.throws(() => store.publish({id:'v1', topology:{id:'x',version:'x',entry:[],nodes:[],edges:[]}}), /immutable/i);
});
