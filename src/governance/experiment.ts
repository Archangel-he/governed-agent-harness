...
export interface CandidateChange {id:string;baselineVersionId:string;candidateVersionId:string;hypothesis:string;evidenceEventIds:string[];sourceExecutionIds:string[]}
...
export interface ExperimentEvaluator {id:string;version:string;judge(input:unknown,output:unknown):TaskOutcome}
...
async function checkTrial(trial:Trial,input:unknown,versionDigest:string,resolver:EvidenceSourceResolver,evaluator:ExperimentEvaluator):Promise<void>{
 ...
 same(evaluator.judge(jsonSnapshot(input),jsonSnapshot(source.output)),trial.outcome,'Evaluator outcome mismatch');
}
export async function runExperiment(
 ...
 const trial=async(version:LocalVersion,input:unknown):Promise<Trial>=>{
  ...
  const output=jsonSnapshot(result.output??null),outcome=jsonSnapshot(evaluator.judge(jsonSnapshot(input),output));
  ...
  await checkTrial(row,input,evidenceDigest(version),resolver,evaluator);return row;
 };
 for(const c of jsonSnapshot(cases)){
  const baseline=await trial(base,c.input),candidateTrial=await trial(next,c.input);
  ...
  rows.push({id:c.id,input:c.input,baseline,candidate:candidateTrial});
 }
 const evidence:Evidence={candidate:jsonSnapshot(candidate),baselineDigest,candidateDigest,profile:jsonSnapshot(profile),evaluator:{id:evaluator.id,version:evaluator.version,implementationDigest:evidenceDigest(evaluator.judge.toString())},sources,cases:rows};
...
export async function promoteCandidate(versions:LocalVersionStore,report:ExperimentReport,approved:boolean,resolver:EvidenceSourceResolver,evaluator:ExperimentEvaluator):Promise<{versionId:string;rollbackVersionId:string;evidenceDigest:string;releaseRef:string}>{
 ...
 for(const row of report.cases){
  ...
  await checkTrial(row.baseline,row.input,report.baselineDigest,resolver,evaluator);
  await checkTrial(row.candidate,row.input,report.candidateDigest,resolver,evaluator);
