...
import type {EvaluationRubric} from './llm-evaluation.js';
...
export interface VersionedEvaluation {version:string;evaluator:ExperimentEvaluator;dataset:EvaluationDataset;gates:EvaluationProfile;llmRubric?:EvaluationRubric}
export interface FrozenEvaluation {version:string;evaluator:ExperimentEvaluator;dataset:Readonly<EvaluationDataset>;gates:Readonly<EvaluationProfile>;llmRubric?:Readonly<EvaluationRubric>}
...
export function freezeEvaluation(definition:VersionedEvaluation):{definition:FrozenEvaluation;digest:string} {
 ...
 const llmRubric=definition.llmRubric?freeze(jsonSnapshot(definition.llmRubric)):undefined;
 const digest=evidenceDigest({version:definition.version,evaluator:{id:definition.evaluator.id,version:definition.evaluator.version,implementationDigest:evidenceDigest(definition.evaluator.judge.toString())},dataset,gates,...(llmRubric?{llmRubric}:{})});
 const evaluator=Object.freeze({...definition.evaluator});
 const frozen={version:definition.version,evaluator,dataset:freeze(dataset),gates:freeze(gates),...(llmRubric?{llmRubric}: {})} as FrozenEvaluation;
