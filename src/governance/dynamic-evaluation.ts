...
export class DynamicEvaluationStore {
 ...
 recordDecision(input:Omit<DecisionCase,'id'> & {id?:string}):DynamicCase {
  ...
  const item={...input,id,status:'pending' as const};this.items.set(id,item);return Object.freeze({...item});
 }
 recordFeedback(feedback:FeedbackEvent):DynamicCase {
  ...
  const next={...item,feedback:Object.freeze({...feedback}),status:'scored' as const};this.items.set(item.id,next);return Object.freeze({...next});
 }
 ...
 snapshot(now=Date.now()):DynamicDatasetSnapshot {
  ...
  const dataset={id:this.id,version:this.version,cases:Object.freeze(cases)};return Object.freeze({...dataset,snapshotAt:now,digest:evidenceDigest(dataset)});
 }
